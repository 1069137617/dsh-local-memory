...
const clipLimit = (n: number | undefined, cfg: Config): number =>
  Math.max(1, Math.min(Math.trunc(n ?? cfg.searchLimit ?? 8), 32))
  ...
  execute: async (args, exec) => {
    ...
    try {
      if (a.action === 'add') {
        ...
        // 并发/异步入队保护：add 前后各取快照，按 id 差集拿本次新建条目（可能不止一条），
        // 回执 revision 取末条所在权威快照；无差集时不再盲信末位（防并发误读）。
        const idsBefore = new Set(store.snapshot().entries.map((e) => e.id))
        const snap = store.add(
          { text: a.text ?? '', scope, ...(a.importance ? { importance: a.importance } : {}), ...(a.tags ? { tags: a.tags } : {}) },
          'agent', c.entryMaxChars,
        )
        const fresh = snap.entries.filter((e) => !idsBefore.has(e.id))
        const last = fresh[fresh.length - 1] ?? snap.entries[snap.entries.length - 1]
        if (!last) return fail('add produced no entry')
        return {
          completion: 'committed' as const,
          id: last.id,
          ...(fresh.length > 1 ? { alsoAdded: fresh.slice(0, -1).map((e) => e.id) } : {}),
          revision: snap.revision,
        }
      }
      if (a.action === 'replace' || a.action === 'remove') {
        ...
        const snap = a.action === 'replace'
          ? store.update(entry.id, { text: spliceText(entry, a), ...(a.importance ? { importance: a.importance } : {}) }, undefined, c.entryMaxChars)
...
// replace 语义 = 宿主 edit 工具的字面替换：id 定位 → text 为整条新文本；
// old_text 定位 → text 替换 old_text 在条目内的首次出现（冻结测试钉死：'alpha one' --alpha→'alpha two'
// 结果必须是 'alpha two one'，后续 old_text='one' 才能命中两条并判 ambiguous）。
const spliceText = (entry: MemoryEntry, a: { id?: string; old_text?: string; text?: string }): string => {
  ...
  if (at < 0) return a.text ?? entry.text
